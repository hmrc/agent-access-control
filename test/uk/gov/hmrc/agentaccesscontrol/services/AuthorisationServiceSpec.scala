...
class AuthorisationServiceSpec extends UnitSpec {
  ...
  trait Setup {
    ...
    protected val mockEnrolmentStoreProxyAuthorisationService: EnrolmentStoreProxyAuthorisationService =
  ...
  "AuthorisationService.isAuthorisedForSa" when {
    "nonMtdAgentAuthDetails provided" should {
      "return NoRelationship if user is not authorised for SA in ESP" in new Setup {
        ...
        mockEnrolmentStoreProxyAuthorisationService
          .isAuthorisedForSaInEnrolmentStoreProxy(credId, saUtr)
          .returns(
            Future
              .successful(false)
          )

        await(TestService.isAuthorisedForSa(agentCode, saUtr, nonMtdAuthDetails)) mustBe AccessResponse.NoRelationship
      }
      "return NoRelationship if user is authorised for SA in ESP, but not authorised for SA in CESA" in new Setup {
        ...
        mockDesAuthorisationService.isAuthorisedInCesa(agentCode, saAgentRef, saUtr).returns(Future.successful(false))
      ...
      "return Authorised if user is authorised in both ESP and CESA" in new Setup {
        ...
        mockDesAuthorisationService.isAuthorisedInCesa(agentCode, saAgentRef, saUtr).returns(Future.successful(true))

        await(TestService.isAuthorisedForSa(agentCode, saUtr, nonMtdAuthDetails)) mustBe AccessResponse.Authorised
      }
      "return an Error if ESP throws an error" in new Setup {
        ...
        intercept[BadRequestException] {
          await(TestService.isAuthorisedForSa(agentCode, saUtr, nonMtdAuthDetails))
        }
      }
      "return an Error if CESA throws an error" in new Setup {
        ...
        mockDesAuthorisationService.isAuthorisedInCesa(agentCode, saAgentRef, saUtr).returns(badRequestException)
    ...
    "mtdAgentAuthDetails provided" should {
      ...
      "return an exception if there is an error when calling ESP to GetDelegatedAgentUserIds" in new Setup {
        mockEnrolmentStoreProxyAuthorisationService.getDelegatedAgentUserIdsFor(saUtr).returns(badRequestException)
      ...
      "return NoRelationship if no agent mappings are found" in new Setup {
        ...
        mockEnrolmentStoreProxyAuthorisationService
          .getDelegatedAgentUserIdsFor(saUtr)
          .returns(Future.successful(Set(AgentUserId("cred1"))))
        mockMappingConnector
        ...
        mockEnrolmentStoreProxyAuthorisationService.getAgentUserIdsFor(List.empty).returns(Future.successful(Seq.empty))
      ...
      "return NoRelationship if no agent user ids are found in Agent Mappings or ESP" in new Setup {
        ...
        mockMappingConnector
          .getAgentMappings("sa", arn)
          .returns(Future.successful(AgentReferenceMappings(List(AgentReferenceMapping(arn.value, saAgentRef.value)))))
        mockEnrolmentStoreProxyAuthorisationService
          .getAgentUserIdsFor(List(saAgentRef))
      ...
      "return an exception if there is an error when calling ESP GetAgentUserIds" in new Setup {
        ...
        mockMappingConnector
          .getAgentMappings("sa", arn)
          .returns(Future.successful(AgentReferenceMappings(List(AgentReferenceMapping(arn.value, saAgentRef.value)))))
        mockEnrolmentStoreProxyAuthorisationService.getAgentUserIdsFor(List(saAgentRef)).returns(badRequestException)
      ...
      "return NoRelationship if userIds found, but do not match" in new Setup {
        ...
        mockEnrolmentStoreProxyAuthorisationService
          .getAgentUserIdsFor(List(saAgentRef))
          .returns(Future.successful(Seq((saAgentRef, Set(AgentUserId("cred2"))))))
      ...
      "return NoRelationship if userIds found, but not authorised in CESA" in new Setup {
        ...
        mockEnrolmentStoreProxyAuthorisationService
          .getAgentUserIdsFor(List(saAgentRef))
          .returns(Future.successful(Seq((saAgentRef, Set(AgentUserId("cred1"))))))
      ...
      "return Authorised if the first agent user id is not authorised, but the second is" in new Setup {
        ...
        mockEnrolmentStoreProxyAuthorisationService
          .getDelegatedAgentUserIdsFor(saUtr)
          .returns(Future.successful(Set(AgentUserId("cred1"), AgentUserId("cred2"))))
        mockMappingConnector
          .getAgentMappings("sa", arn)
          .returns(
            Future.successful(
              AgentReferenceMappings(
                List(AgentReferenceMapping(arn.value, saAgentRef.value), AgentReferenceMapping("nra", "654ABC"))
              )
            )
          )
        mockEnrolmentStoreProxyAuthorisationService
          .getAgentUserIdsFor(List(saAgentRef, SaAgentReference("654ABC")))
          .returns(
            Future.successful(
              Seq((saAgentRef, Set(AgentUserId("cred1"))), (SaAgentReference("654ABC"), Set(AgentUserId("cred2"))))
            )
          )
        mockDesAuthorisationService.isAuthorisedInCesa(agentCode, saAgentRef, saUtr).returns(Future.successful(false))

        mockDesAuthorisationService
          .isAuthorisedInCesa(agentCode, SaAgentReference("654ABC"), saUtr)
      ...
      "return Authorised if the first agent user id gets an error from CESA, but the second gets an authorised response" in new Setup {
        ...
        mockDesAuthorisationService.isAuthorisedInCesa(agentCode, saAgentRef, saUtr).returns(failedResponse)
    ...
    "no agent reference found for agent" should {
      "return an AccessResponse Error" in new Setup {
        ...
        val result: AccessResponse =
          await(TestService.isAuthorisedForSa(agentCode, saUtr, AuthDetails(None, None, "ggId", None, None)))

        result mustBe AccessResponse.Error(s"No 6 digit agent reference found for agent $agentCode")
      }
    }
  }
  ...
  "AuthorisationService.isAuthorisedForPaye" when {
    "not authorised in ESP" should {
      "return NoRelationship" in new Setup {
        mockAuditService
          ...
          .returns(Future.successful(Success))
        mockEnrolmentStoreProxyAuthorisationService
          .isAuthorisedForPayeInEnrolmentStoreProxy(credId, empRef)
          .returns(Future.successful(false))

        val result: AccessResponse = await(TestService.isAuthorisedForPaye(agentCode, empRef, nonMtdAuthDetails))
    ...
    "an error is thrown by ESP" should {
      "propagate the error" in new Setup {
        ...
        intercept[BadRequestException] {
          await(TestService.isAuthorisedForPaye(agentCode, empRef, nonMtdAuthDetails))
        }
      }
    }
    ...
    "agent auth details not provided" should {
      "return an AccessResponse Error" in new Setup {
        val result: AccessResponse =
          await(TestService.isAuthorisedForPaye(agentCode, empRef, AuthDetails(None, None, "", None, None)))
  ...
  "AuthorisationService.isAuthorisedForAfi" when {
    ...
    "auth details are not provided" should {
      "return NoRelationship" in new Setup {
        val result: AccessResponse = await(
          TestService
            .isAuthorisedForAfi(agentCode, nino, AuthDetails(None, None, "ggId", None, None))
        )
