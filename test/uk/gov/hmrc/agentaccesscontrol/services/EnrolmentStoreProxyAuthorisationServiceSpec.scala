...
class EnrolmentStoreProxyAuthorisationServiceSpec extends UnitSpec {
  ...
  "isAuthorisedForSaInEnrolmentStoreProxy" should {
    "return true if the client is assigned to the agent credential" in new Setup {
      mockEnrolmentStoreProxyConnector
        .getIRSADelegatedUserIdsFor(saUtr)
        .returns(Future.successful(Set(AgentUserId("000111333"))))

      val result = await(TestService.isAuthorisedForSaInEnrolmentStoreProxy("000111333", saUtr))
    ...
    "return true if there is more than one agent assigned to the client" in new Setup {
      mockEnrolmentStoreProxyConnector
        .getIRSADelegatedUserIdsFor(saUtr)
        .returns(Future.successful(Set(AgentUserId("000111333"), AgentUserId("000111444"))))

      val result = await(TestService.isAuthorisedForSaInEnrolmentStoreProxy("000111444", saUtr))
    ...
    "return false if the client is not assigned to the agent credential" in new Setup {
      mockEnrolmentStoreProxyConnector
        .getIRSADelegatedUserIdsFor(saUtr)
        .returns(Future.successful(Set(AgentUserId("000111444"))))
    ...
    "throw exception if enrolment store proxy fails" in new Setup {
      mockEnrolmentStoreProxyConnector.getIRSADelegatedUserIdsFor(saUtr).returns(Future.failed(new RuntimeException()))

      an[RuntimeException] should be thrownBy await(
        TestService.isAuthorisedForSaInEnrolmentStoreProxy("NonMatchingCred", saUtr)
      )
    }
  }
  ...
  "isAuthorisedForPayeInEnrolmentStoreProxy" should {
    "return true if the client is assigned to the agent credential" in new Setup {
      mockEnrolmentStoreProxyConnector
        .getIRPAYEDelegatedUserIdsFor(empRef)
        .returns(Future.successful(Set(AgentUserId("000111333"))))

      val result = await(TestService.isAuthorisedForPayeInEnrolmentStoreProxy("000111333", empRef))
    ...
    "return true if there is more than one agent assigned to the client" in new Setup {
      ...
      val result = await(TestService.isAuthorisedForPayeInEnrolmentStoreProxy("000111444", empRef))
    ...
    "throw exception if enrolment store proxy fails" in new Setup {
      ...
      an[RuntimeException] should be thrownBy await(
        TestService.isAuthorisedForPayeInEnrolmentStoreProxy("NonMatchingCred", empRef)
      )
    }
  }
  ...
  "getAgentUserIdsFor" should {
    "Return multiple Agent User Ids if they were found" in new Setup {
      mockEnrolmentStoreProxyConnector
        ...
        .returns(Future.successful(Set(AgentUserId("000111444"), AgentUserId("444111000"))))
    ...
    "Throw an exception if the ESP call fails" in new Setup {
      ...
      an[RuntimeException] should be thrownBy await(TestService.getAgentUserIdsFor(SaAgentReference("ref1")))
    }
  }
  ...
  "getAgentUserIdsFor - sequence" should {
    "Populate both lists if both references found results" in new Setup {
      ...
      mockEnrolmentStoreProxyConnector
        ...
        .returns(Future.successful(Set(AgentUserId("999111444"), AgentUserId("444111999"))))

      val result = await(TestService.getAgentUserIdsFor(Seq(SaAgentReference("ref1"), SaAgentReference("ref2"))))

      result mustBe Seq(
        (SaAgentReference("ref1"), Set(AgentUserId("000111444"), AgentUserId("444111000"))),
        (SaAgentReference("ref2"), Set(AgentUserId("999111444"), AgentUserId("444111999")))
      )
    }
    ...
    "Throw an exception if the ESP call fails" in new Setup {
      ...
      an[RuntimeException] should be thrownBy await(
        TestService.getAgentUserIdsFor(Seq(SaAgentReference("ref1"), SaAgentReference("ref2")))
      )
    }
  }
