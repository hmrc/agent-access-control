    ...
    authConnector.currentAuthDetails() flatMap {
      case Some(agentAuthDetails @ AuthDetails(_, Some(arn), _, _, _)) =>
        hasRelationship(arn, vrn) map { result =>
          ...
          if (result)
            authorised(s"Access allowed for agentCode=$agentCode arn=${arn.value} client=${vrn.value}")
          else
            notAuthorised(s"Access not allowed for agentCode=$agentCode arn=${arn.value} client=${vrn.value}")
        }
